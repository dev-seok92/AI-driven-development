...
**4. 헤더 컴포넌트**
   - **전체 레이아웃**: 화면 상단에 고정되는 고정 헤더로, 모든 페이지에서 일관된 네비게이션을 제공.
   - **로고**: 
     - 좌측 상단에 배치하여 브랜드 아이덴티티 강조
     - 클릭 시 메인 화면으로 이동하는 링크 기능 제공
     - 간결하고 인식하기 쉬운 텍스트 또는 아이콘 형태로 디자인
   - **네비게이션 메뉴**: 
     - 중앙 또는 우측에 배치하여 주요 페이지 간 이동 지원
     - "내 갤러리" 링크: 사용자의 개인 갤러리 페이지로 이동
     - "이미지 생성" 링크: 이미지 생성 페이지로 이동
   - **스타일 가이드**:
     - 배경색: 깔끔한 흰색 배경(#FFFFFF)
     - 텍스트 색상: 다크 그레이(#333333)
     - 네비게이션 링크 호버 시: 브랜드 메인 색상(#4A90E2)으로 변경
     - 하단 경계선: 연한 회색(#E0E0E0)으로 본문과 구분
   - **반응형 디자인**: 모바일 화면에서는 햄버거 메뉴로 전환하여 공간 효율성 확보
